...
export class ReporteFiltroComponent implements OnInit {
  ...
  constructor(private api: TramiteService, private fs: FileSave, private auth:AuthService) {}

  displayFn = (r?: UsuarioApp) => (r ? r.nombreUsuario : undefined);
  ngOnInit() {
    ...
    this.nombreUsuario.setValue({nombreUsuario:userActual.nombreUsuario,
                                  loginUsuario:userActual.loginUsuario});
    this.dateTimeRange=[this.reportef.fechaInicio, this.reportef.fechaFin];
  }
  buscarUsuario(value): Observable<UsuarioApp[]> {
  ...
  descargarPdf(){
    this.api.getReportePdf(this.reportef)
