...
export class DocumentoAnexoInternoComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.getArchivos(1);
    this.getArchivos(2);
    if (this.screenAtencion) this.getArchivos(3);
  }
  ...
  adjuntar(tipo: number) {
    const dialogRef = this.dialog.open(DetalleAdjuntarInternoComponent, {
      ...
      data: {
        ...
        codigoDocumentoAdjuntoReemplazo:
          tipo == 1 ? this.archivoPrincipal.codigoDocumentoAdjunto : null,
  ...
  derivar() {
    ...
    const dialogRef = this.dialog.open(DetalleAprobacionInternoComponent, {
      ...
      width: '50vw',
      height: '100vh',
      data: { doc: this.doc, tipo: 'Derivar' },
    });
    dialogRef.afterClosed().subscribe((complete) => {
  ...
  abrirVisor(archivoPrincipal: ArchivoDocumento) {
    ...
    dialogRef.afterClosed().subscribe((c) => {
      //console.log(c)
      if (c) this.archivoPrincipal.codigoDocumentoAdjunto = c;
      this.getArchivos(1);
    });
  }
  ...
  aprobar() {
    ...
    const dialogRef = this.dialog.open(DocumentoInternoVisorComponent, {
      width: '43vw',
      height: '100vh',
      data: data,
    });
  ...
  derivarPrincipal() {
    ...
    dialogRef.afterClosed().subscribe((c) => {
      ...
      } else this.getArchivos(1);
    });
  }

  verMemo() {
    //console.log(this.archivoPrincipal);
    const dialogRef = this.dialog.open(ReporteMemoComponent, {
      width: '54vw',
      height: '90vh',
      data: {
        codigoDocumento: this.doc.codigoDocumentoTramite,
  ...
  descargarMemo() {
    if (this.archivoPrincipal.codigoDocumentoAdjunto)
      this.api
        .descargarArchivo(
          this.archivoPrincipal.codigoDocumento,
          this.archivoPrincipal.codigoDocumentoAdjunto
        )
        .subscribe((blob) => this.setFile(this.archivoPrincipal, blob));
    else
      this.api
        .getReporteMemo(this.archivoPrincipal.codigoDocumento)
        .subscribe((blob) => this.setFile(this.archivoPrincipal, blob));
  }
