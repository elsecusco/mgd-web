...
export class DocumentoInternoVisorComponent implements OnInit {
  ...
  cellDerivar = false;
  constructor(
    public dialogRef: MatDialogRef<DocumentoInternoVisorComponent>,
  ...
  initForm() {
    this.form = this.fb.group({
      ...
      titulo: [
        this.data.nombreFile,
        [Validators.required, Validators.maxLength(110)],
      ],
      ...
      codigoDocumentoAdjuntoReemplazo: [
        this.data.hasOwnProperty('codigoDocumentoAdjuntoReemplazo')
          ? this.data.codigoDocumentoAdjuntoReemplazo
          : null,
      ],
      razon: [
        this.data.nombreDerivacion || this.data.estadoDerivacion == 'FD'
          ? 'FD'
          : '',
        Validators.required,
      ],
  ...
  loadPdf() {
    this.api
      .descargarArchivo(
        this.data.codigoDocumento,
        this.data.codigoDocumentoAdjunto
      )
  ...
  aprobarArchivo() {
    this.api.aprobarValidacion(this.form.value).subscribe((event) => {
  ...
  guardarArchivoFirmado() {
    this.api.guardarArchivoFirmado(this.form.value).subscribe(
      (res) => {
        ...
        this.isSave = false;
        this.firmando = false;
        if (this.data.tipo == 'Derivar') this.derivarArchivo();
        else this.aprobarArchivo();
      },
      (_err) => {
        this.isSave = false;
        this.firmando = false;
