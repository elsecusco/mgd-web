...
import { DocumentoInterno } from 'src/app/@models/tramite/documento-interno';
...
const defaultStateBandejaInterno = {
  ...
  listaBuzones: [],
  destinatarioGerente: new Destinatario(),
  destinatarioJefe: new Destinatario(),
...
export class BandejaInternoState {
  ...
  public static loadDocuments(ctx: Context, action: FiltroAction) {
    ...
    return this.api
      .getBandejaDocumentoInterno(buzonUsuario.loginUsuarioBuzon, bandejaFiltro)
      .pipe(
        tap((d) => {
          let bandejas = new BandejasInternas();
          // bandejas[bandejaFiltro.bandeja] = d.bandeja;
          bandejas.setAtributes(bandejaFiltro.bandeja, d.bandeja); //--- set
          ctx.patchState({
            bandejas: bandejas,
  ...
  public static searchDocuments(ctx: Context, action: BuscarAction) {
    ...
    return this.api
      .buscarBandejaInterna(buzonUsuario.loginUsuarioBuzon, bandejaBuscar)
      .pipe(
        tap((b) => {
          const bandejaActiva =
            b.e.length > 0
  ...
  public static loadDestinatarios(ctx: Context) {
    ...
    return this.api.retornarJefeyGerente().pipe(
      tap((djg) => {
        ctx.patchState({
          destinatarioGerente: djg.gerente.length == 0 ? null : djg.gerente[0],
