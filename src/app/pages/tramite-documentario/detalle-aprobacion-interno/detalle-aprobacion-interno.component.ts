...
export class DetalleAprobacionInternoComponent implements OnInit {
  ...
  constructor(
    ...
    @Inject(MAT_DIALOG_DATA) public data:DataAprobacion                       
      ...
      if(data.doc.tipoDerivacion=='D'){
        ...
        if(data.tipo=="Derivar")
        this.derivarSubir=true
      else
        this.aprovarSubir=true
      }
  }
  ...
  initForm() {
    this.form = this.fb.group({
      ...
      codigoDocumento: [this.data.doc.codigoDocumentoTramite, Validators.required],
  ...
  derivarArchivo(){
    ...
    if (this.file instanceof File) {
      this.api.derivarArchivoFinal(frm, this.file).subscribe(
        event => {
          this.progress = event.percent;
          if (event.status === UploadStatus.Completed && event.data) {
            notifyOk(event.data.mensaje);
  ...
  derivarDePrincipal(){
    const frm={... this.data.doc, ...this.form.value};
    this.api.derivarDeFinal(frm).subscribe(
        r => {
            notifyOk(r.mensaje);
            this.dialogRef.close(true);
          });
  ...
  aprovarDePrincipal(){
    const frm={... this.data.doc, ...this.form.value};
    this.api.aprovarDeFinal(frm).subscribe(
        r => {
            notifyOk(r.mensaje);
            this.dialogRef.close(true);
          });
