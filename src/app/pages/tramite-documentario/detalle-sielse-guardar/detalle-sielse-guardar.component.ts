...
export class DetalleSielseGuardarComponent implements OnInit {
  ...
  onCodigoSucursalChange(codigoSucursal) {
    this.tiposTecnicoSupervisor=this.tipos.tiposTecnicoSupervisor[codigoSucursal];
  ...
  onClaseOTMantenimientoChange(claseOt){
    this.tiposOTMantenimiento=this.tipos.tiposOTMantenimiento[claseOt];
  }
  guardar(){
    this.api.guardarOTMantenimiento(this.form.value).subscribe(
      res => {
