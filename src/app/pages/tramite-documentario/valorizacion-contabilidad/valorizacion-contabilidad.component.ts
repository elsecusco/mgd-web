...
export class ValorizacionContabilidadComponent implements OnInit {
  ...
  ngOnInit() {}
  valorizacionDetalle() {
    const dialogRef = this.dialog.open(ValorizacionDetalleComponent, {
      width: '850px',
      height: '90hv',
      data: {},
    });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) this.detalleContrato = result;
      //this.clear();
    });
  }
  ...
  addValorizacion() {
    ...
    if (this.nvaValorizacion.serie) {
      ...
      this.changeValorizaciones.emit(
        this.detalleContrato.listValorizaciones!.length
      );
  ...
  save(codigoDocumento: any) {
    this.detalleContrato.codigoDocumento = codigoDocumento;
    this.api
      .guardarValorizacionContabilidad(this.detalleContrato)
      .subscribe((res) => {
        if (res.id == 1) {
