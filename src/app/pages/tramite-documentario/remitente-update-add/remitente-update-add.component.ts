...
export class RemitenteUpdateAddComponent{
  ...
  form: FormGroup;
  tiposRemitente:Observable<TipoRemitente[]>;Tipo
  ...
  ngOnInit() {
     this.tiposRemitente = this.api.tiposRemitente().pipe(
      finalize(()=>{})
    );
    this.initForm();
  }
  ...
  guardarRemitente() {this.saving = true;
      this.api.guardarRemitente(this.form.value).subscribe(
      res => {
