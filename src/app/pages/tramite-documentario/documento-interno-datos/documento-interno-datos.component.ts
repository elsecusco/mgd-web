...
export class DocumentoInternoDatosComponent implements OnInit {
  ...
  @ViewChild(ValorizacionContabilidadComponent) valorizacion:ValorizacionContabilidadComponent;
  ...
  ngOnInit() {
    ...
    this.cargarValorizacionDi()
    this.loaded$.subscribe(l => {
      if (!l) this.loadTipos.emit();
    });
    this.tipos$.subscribe(tipos =>{
      this.tipos = tipos
      this.correlativos = this.tipos.tiposCorrelativos[this.buzonActual.loginUsuarioBuzon];
  ...
  setCodigosTxt(){
    if(this.tipos.tiposDocumentoInterno.length>0 && this.form){
      this.tipoDocumento=
        this.tipos.tiposDocumentoInterno.find(e=>e.codigo===this.doc.codigoTipoDocumento).nombre;
        this.tipoRecepcion=
        this.tipos.tiposRecepcion.find(e=>e.codigo===this.doc.codigoTipoRecepcionDocumento).nombre;
  ...
  cargarValorizacionDi(){
    this.api.listarValorizacion(this.codigoDocumento).subscribe( valorizacion =>{
